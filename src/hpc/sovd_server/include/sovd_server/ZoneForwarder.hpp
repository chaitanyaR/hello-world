...
// For simulation, zone faults are held in an in-process SovdFaultMemory.
...
public:
    ...
    // Inject a fault from SOME/IP event (called by zone event handler)
    void injectFault(sdv::sovd::DtcEntry entry);
